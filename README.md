...
| DBMS Agent | 디엠 | DBMS 운영 가이드 RAG | 8510 |

포트는 로컬/배포 환경 공통이며, host만 환경에 따라 바뀝니다(아래 "로컬 vs 배포" 참고). 카드별 노출 여부(로컬/배포 독립)와 화면상 순서는 ⚙️ 설정 페이지에서 관리합니다. 순서는 숫자를 입력하는 대신 **▲▼로 한 칸씩** 옮기며, 한 번 옮길 때마다 전체 순서가 0부터 다시 매겨집니다(중복·빈 번호가 정리됩니다).
...
배포 기능(⚙️ 설정 페이지의 "🚀 서버 배포")과 🎛️ 오케스트레이터를 쓰려면 `.env.example`을 참고해 `.env`를 직접 만들어야 합니다(git에 포함되지 않음).
...
| `ANTHROPIC_API_KEY` | (선택) 있으면 오케스트레이터가 Claude를 먼저 씁니다 |
| `GEMINI_API_KEYS` | Gemini 키를 **쉼표로 여러 개**. 고시·프니·에리와 같은 규약 |
| `GEMINI_MODEL` | (선택) 기본값 `gemini-2.5-flash` |

오케스트레이터의 LLM은 **Anthropic 키가 있으면 Claude를 먼저 쓰고, 없거나 실패하면 Gemini로 넘어갑니다.** 둘 중 하나는 있어야 동작합니다. Gemini 키는 여러 개를 라운드로빈으로 돌려 쓰고, 한 키가 실패하면 다음 키로 넘어가며, 429가 연속 3회면 전체 할당량 소진으로 보고 멈춥니다(남은 키를 헛되게 순회하지 않기 위함).
...
배포는 **로컬 포털에서만** 합니다. 배포 서버에서 연 ⚙️ 설정 페이지에는 "서버 배포"와 "어댑터 배포" 섹션이 아예 그려지지 않고, 카드 노출 설정도 읽기 전용입니다 — 배포 서버에서 배포를 누르면 자기 자신에게 SSH로 재배포·재시작이 걸려 돌아가던 앱이 끊기고, 노출 설정은 어차피 다음 배포 때 로컬 파일로 덮어써지기 때문입니다.
...
어느 에이전트를 부를지 정하는 것과 답변을 합치는 것에 LLM을 씁니다. 공급자는
`orchestrator_llm.py` 한 곳에만 있고 `call_json`/`call_text` 계약이 고정되어 있어,
Claude에서 Gemini로 바꾸는 데 라우터·실행기·합성기·페이지는 손대지 않았습니다.
설정은 위 "환경 변수"를 참고하세요.
...
어댑터는 `127.0.0.1`에만 바인딩됩니다 — `/ask`에 인증이 없어 밖에서 닿으면
안 되기 때문입니다. 오케스트레이터는 같은 머신에서 부르므로 영향이 없고, 설정
페이지의 상태 조회만 SSH `curl`을 경유합니다. 렉스·폴리 유닛에는 기동 직후
페이지를 한 번 긁는 워머가 붙어 있어 재부팅 후에도 어댑터가 스스로 올라옵니다.

재시작 버튼은 유닛이 쓸 포트를 다른 프로세스가 잡고 있으면 거부합니다 — 기존
프로세스를 남겨둔 채 서비스를 띄우면 5초마다 재시작하는 루프가 되기 때문입니다.
