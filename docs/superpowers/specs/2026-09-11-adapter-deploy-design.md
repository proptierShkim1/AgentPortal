...
6. **재시작(별도 버튼)** — 먼저 포트 점유를 확인하고, 유닛이 쓸 포트를 다른
   프로세스가 잡고 있으면 **재시작을 거부한다**. 그다음 `systemctl --user restart` →
   실패 시 백업 파일명과 롤백 명령을 로그에 찍는다.

포트 가드가 필요한 이유는 실제로 당해봤기 때문이다. 렉스·폴리를 nohup으로 띄워둔 채
서비스를 start하면 9001·9002 바인딩에 실패하고 `Restart=on-failure`가 5초마다
재시도해 **1,600회 넘게 반복**했다. 문서에 "누르지 마세요"라고 적는 것으로는 부족하고,
코드가 거부해야 한다.
...
## 실사용 점검에서 고친 것 (2026-09-11 오후)

1. **어댑터를 루프백에만 바인딩한다.** 배포 서버는 `firewalld` inactive, nftables
   규칙 0개인데 어댑터 4개가 `0.0.0.0`에 떠 있었다. `/ask`에 인증이 없으므로 같은
   망의 누구나 LLM 호출을 태울 수 있는 상태였다. `agent_host.adapter_host()`를
   따로 두어 카드 링크(LAN IP)와 어댑터 호출(127.0.0.1)을 분리했고, 설정 페이지의
   상태 조회는 SSH `curl`로 바꿨다. 설계 §4의 "v1은 LAN 신뢰"는 LAN 안에서만
   닿는다는 전제였는데 그 전제가 성립하지 않았다.
2. **LLM 폴백을 조용히 넘기지 않는다.** Claude 실패가 `print`로만 남아 Streamlit
   버퍼에 갇히고 화면엔 Gemini 429 원문만 떴다. 두 공급자가 다 막히면
   `LLMUnavailable`에 각각의 사유를 담고, 폴백이 성공해도 화면에 경고를 남긴다.
3. **thread 모드 유닛에 워머를 붙인다.** Streamlit은 접속이 있어야 `app.py`를
   실행하므로 재부팅 후 아무도 페이지를 안 열면 어댑터가 뜨지 않는다.
   `ExecStartPost`로 기동 직후 한 번 긁는다. 반드시 `exit 0`으로 끝난다 —
   실패하면 유닛이 failed가 되어 `Restart=on-failure` 루프에 걸린다.
